...
from utilities.read_data import getCSVData   # da bi se koristilo metoda za citanje CSV file

@ddt
class CSVDataLoginTests(unittest.TestCase): # u zagradu se upusije kad se importuje unittest
    @pytest.mark.run(order=1)
    @data(*getCSVData("/Users/Dusan/workspace_python/pitomi/testdata.csv"))
    @unpack

    def test_resultList(self, nekaSlova, prefixCode):
        ...
        sleep(3)
        searchGas=calcDSL.Gasnesto(nekaSlova)
        searchTariffs=calcDSL.DSLResultList(prefixCode)
